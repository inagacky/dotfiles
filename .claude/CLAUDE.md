## 言語設定
**重要** このプロジェクトでは、Claude codeは必ず日本語で返却してください。技術用語は必要に応じて英語のまま使用可能です。
...
### 3. 自己改善ループ
- ユーザーから修正を受けたら必ず プロジェクトの`tasks/lessons.md` にそのパターンを記録する
...
1. **まず計画を立てる**：チェック可能な項目として プロジェクトの`tasks/todo.md` に計画を書く
2. **計画を確認する**：実装を開始する前に確認する
3. **進捗を記録する**：完了した項目を随時マークしていく
4. **変更を説明する**：各ステップで高レベルのサマリーを提供する
5. **結果をドキュメント化する**：プロジェクトの`tasks/todo.md` にレビューセクションを追加する
6. **学びを記録する**：修正を受けた後に プロジェクトの`tasks/lessons.md` を更新する
